...
#include "bmsbel\bmx2wav_exception.h"

#include "bmsbel\bmx2wav_wav.h"
...
static bool
make_parent_directory_recursive( const std::string& filepath )
{
  if ( TtPath::IsDirectory( filepath ) ) {
    return true;
  }
  if ( TtPath::FileExists( filepath ) ) {
    return false;
  }
  std::string parent = TtPath::DirName( filepath );
  if ( NOT( make_parent_directory_recursive( parent ) ) ) {
    return false;
  }
  return ::CreateDirectory( filepath.c_str(), NULL );
}
...
{
  try {
    if ( NOT( make_parent_directory_recursive( TtPath::DirName( filename ) ) ) ) {
      throw TtFileOpenException( filename, ENOENT );
    }
    ...
    throw WavFileWriteException( filename );
  }
}
