...
{
  if ( file_ == NULL ) {
    throw TtFileOpenException( filename, errno );
  }
}
...
void
WavFileReader::Read( TtTemporalyArray<unsigned char>& buffer )
{
...
// -- WavMaker::Reactor --------------------------------------------------
WavMaker::Reactor
WavMaker::Reactor::DefaultObject;

void
WavMaker::Reactor::AtReadStart( void )
{
}

void
WavMaker::Reactor::AtHeaderRead( void )
{
}

void
WavMaker::Reactor::AtOneTickRead( void )
{
}

void
WavMaker::Reactor::AtReadEnd( void )
{
}

// -- WavMaker -----------------------------------------------------------
WavMaker::WavMaker( Reactor& reactor, bool not_use_filter ) :
reactor_( reactor ),
...
reactor_( WavMaker::Reactor::DefaultObject ),
not_use_filter_( not_use_filter )
{
}
...
WavMaker::MakeNewWavFromWavFile( const std::string& filename, bool overlook_error )
{
  ...
  try {
    ...
    if ( reader.ReadInteger() != HQWav::WAVE_HEADER && NOT( overlook_error ) ) {
      throw InvalidWavFileFormatException( filename, "WAVE header" );
    }
    ...
    if ( reader.Read2Byte() != HQWav::FMT_ID ) {
      throw InvalidWavFileFormatException( filename, "fmt id" );
    }

    // channel count
    int channel_count = reader.Read2Byte();
    if ( channel_count != 1 && channel_count != 2 ) {
      throw UnsupportedWavFileFormatException( filename, "channel count is " + TtUtil::ToStringFrom( channel_count ) );
    }
    ...
    if ( bit_rate != 8 && bit_rate != 16 && bit_rate != 24 ) {
      throw UnsupportedWavFileFormatException( filename, "bit rate is " + TtUtil::ToStringFrom( bit_rate ) );
    }

    // 細かいチェック
    if ( block_size != (bit_rate / 8) * channel_count && NOT( overlook_error ) ) {
      throw InvalidWavFileFormatException( filename, "block size, bit rate, and channel count" );
    }
    if ( byte_per_second != frequency * block_size && NOT( overlook_error ) ) {
      throw InvalidWavFileFormatException( filename, "byte per second, frequency, and block size" );
    }
    ...
    if ( data_size % ((bit_rate / 8) * channel_count) != 0  ) {
      if ( NOT( overlook_error ) ) {
        throw InvalidWavFileFormatException( filename, "data size can not be divided by block size" );
      }
      data_size -= data_size % ((bit_rate / 8) * channel_count );
    }
    reactor_.AtHeaderRead();
    ...
    TtTemporalyArray<unsigned char> buffer( data_size );
    reader.Read( buffer );
...
DataReader::EightBitWav::EightBitWav( TtTemporalyArray<unsigned char>& buffer, int frequency ) :
DataReader::Base( frequency ),
buffer_( buffer ),
current_( 0 )
{
...
DataReader::SixteenBitWav::SixteenBitWav( TtTemporalyArray<unsigned char>& buffer, int frequency ) :
DataReader::Base( frequency ),
buffer_( buffer ),
current_( 0 )
{
...
DataReader::TwentyFourBitWav::TwentyFourBitWav( TtTemporalyArray<unsigned char>& buffer, int frequency ) :
DataReader::Base( frequency ),
buffer_( buffer ),
current_( 0 )
{
