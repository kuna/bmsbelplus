...
namespace {
  ...
  public:
    ...
    file_( fopen( filename.c_str(), "rb" ) ) {
      if ( file_ == NULL ) {
        throw TtFileOpenException( filename_, errno );
      }
    }
  ...
  public:
    ...
    file_holder_( file_holder ) {
      if ( ov_open( file_holder.GetPtr(), &vf_, NULL, 0 ) < 0 ) {
        throw OggFileOpenException( file_holder.GetFileName() );
      }
    }
...
DataReader::SixteenBitOgg::ReadOneData( void )
{
  ...
  if ( current_ + 1 != last_read_size_ ) {
    ...
    if ( NOT( this->DataRemains() ) ) {
      throw OggFileReadException( ogg_vorbis_file_holder_.GetFileHolder().GetFileName() );
    }
...
WavMaker::MakeNewWavFromOggFile( const std::string& filename )
{
  ...
  if ( vorbis_info == NULL ) {
    throw OggFileReadException( filename );
  }
