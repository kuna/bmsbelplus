...
const BmsWord BmsWord::MIN(BmsConst::WORD_MIN_VALUE);
const BmsWord BmsWord::MAX(BmsConst::WORD_MAX_VALUE);

bool
BmsWord::CheckConstruction(const char* str)
{
	return BmsUtil::IsHex36(str[0]) && BmsUtil::IsHex36(str[1]);
}

bool
BmsWord::CheckConstruction(const std::string& str)
{
	return BmsWord::CheckConstruction(str.c_str());
}
...
BmsWord::BmsWord(int value) :
value_(value)
{
}

BmsWord::BmsWord(char msb, char lsb) :
value_(0)
{
	this->SetByChar(msb, lsb);
}

BmsWord::BmsWord(const char* str) :
value_(0)
{
	this->SetByChar(str[0], str[1]);
}

BmsWord::BmsWord(const std::string& str) :
value_(0)
{
	this->SetByChar(str.c_str()[0], str.c_str()[1]);
}

BmsWord::BmsWord(const wchar_t* str) :
value_(0)
{
	this->SetByChar(str[0], str[1]);
}

BmsWord::BmsWord(const std::wstring& str) :
value_(0)
{
	this->SetByChar(str.c_str()[0], str.c_str()[1]);
}
...
void
BmsWord::SetByWord(const BmsWord& word)
{
	value_ = word.value_;
}

void
BmsWord::SetByInteger(int value)
{
	value_ = value;
}

void
BmsWord::SetByChar(char msb, char lsb)
{
	if (BmsUtil::IsNotHex36(msb) || BmsUtil::IsNotHex36(lsb)) {
		throw BmsInvalidCharUsedAsWordException(msb, lsb);
	}
	value_ = BmsUtil::CharacterToNumber(msb) * 36 + BmsUtil::CharacterToNumber(lsb);
}


int
BmsWord::ToInteger(void) const
{
	return value_;
}

char
BmsWord::GetMsb(void) const
{
	return this->ToCharPtr()[0];
}

char
BmsWord::GetLsb(void) const
{
	return this->ToCharPtr()[0];
}

const char*
BmsWord::ToCharPtr(void) const
{
	this->CheckValid();
...
std::string
BmsWord::ToString(void) const
{
	return std::string(this->ToCharPtr());
}
...
{
	if (value_ < BmsConst::WORD_MIN_VALUE ||
		value_ > BmsConst::WORD_MAX_VALUE) {
		throw BmsInvalidWordValueUsedException(value_);
	}
}

void
BmsWord::operator ++(void)
{
	++value_;
}

void
BmsWord::operator ++(int)
{
	value_++;
}
...
bool
BmsWord::equal(const BmsWord &obj) const
{
	return value_ == obj.value_;
}

bool
BmsWord::operator ==(const BmsWord &obj) const
{
...
BmsWord::operator !=(const BmsWord &obj) const
{
	return NOT(this->equal(obj));
}


int
BmsWord::compare(const BmsWord &obj) const
{
	return value_ - obj.value_;
}
...
std::ostream&
operator <<(std::ostream& os, const BmsWord& word)
{
	os << word.ToCharPtr();
	return os;
}
